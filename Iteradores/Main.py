mport pprint
from Modules import List_I as l_iter
from Modules import Set_I as s_comp
from Modules import Dict_I as d_iter
from Modules import Sp_I as s_op
import csv
import os
    ...
    """)
        ...
        5. Crear una lista de diccionarios de todos los registros hayan ganado las 3 medallas 
            ...
            if opciones_internas == 1:
                iterable_print(l_iter.all_values_dict(data_csv))
            elif opciones_internas == 2:
                iterable_print(l_iter.tuples_countries(data_csv))
            elif opciones_internas == 3:
                iterable_print(l_iter.total_medals_over_40(data_csv))
            elif opciones_internas == 4:
                iterable_print(l_iter.total_medals_above_20(data_csv))
            elif opciones_internas == 5:
                iterable_print(l_iter.each_medal_teams(data_csv))
            else:
        ...
        5. Crear un diccionario de diccionarios con todos los registros donde tenga minimo una medalla de plata
            ...
            os.system(var)
            if opciones_internas == 1:
                print_nice(d_iter.medals_only_dict(data_csv))
            elif opciones_internas == 2:
                print_nice(d_iter.total_medals_above_10(data_csv))
            elif opciones_internas == 3:
                print_nice(d_iter.all_values_dict(data_csv))
            elif opciones_internas == 4:
                print_nice(d_iter.at_least_one_gold_medal(data_csv))
            elif opciones_internas == 5:
                print_nice(d_iter.at_least_one_silver_medal(data_csv))
            else:
                print("Opción incorrecta")
            aux_char = input('Presione cualquier tecla para continuar').split(" ")[0]
            os.system(var)
        ...
        5. Crear un conjunto de todos los registros donde sus elementos esten formados por una tupla (`Country`,`Rank Total`) 
            ...
            os.system(var)
            if opciones_internas == 1:
                iterable_print(s_comp.country_gold_medals(data_csv))
            elif opciones_internas == 2:
                iterable_print(s_comp.country_silver_medals(data_csv))
            elif opciones_internas == 3:
                iterable_print(s_comp.country_bronce_medals(data_csv))
            elif opciones_internas == 4:
                iterable_print(s_comp.all_values_set(data_csv))
            elif opciones_internas == 5:
                iterable_print(s_comp.country_all_medals(data_csv))
            else:
                print("Opción incorrecta")
            aux_char = input('Presione cualquier tecla para continuar').split(" ")[0]
            os.system(var)
        ...
        5. Mediante operacion de conjuntos obtener que paises se encuentran en el ejercicio 2 de set comprehensions pero no en el conjunto 3
            ...
            os.system(var)
            if opciones_internas == 1:
                print_nice(s_op.countries_with_all_medals(set(s_comp.country_gold_medals(data_csv)),set(s_comp.country_silver_medals(data_csv)),set(s_comp.country_bronce_medals(data_csv))))

            elif opciones_internas == 2:
                print_nice(s_op.countries_without_gold_medals_but_silver_and_bronce(set(s_comp.country_silver_medals(data_csv)),set(s_comp.country_bronce_medals(data_csv)),set(s_comp.country_gold_medals(data_csv))))

            elif opciones_internas == 3:
                print_nice(s_op.countries_without_gold_and_silver_but_bronce(set(s_comp.country_bronce_medals(data_csv)),set(s_comp.country_silver_medals(data_csv)),set(s_comp.country_gold_medals(data_csv))))

            elif opciones_internas == 4:      
                print_nice(s_op.exclusion_ex_2_between_3_sets(set(s_comp.country_silver_medals(data_csv)),set(s_comp.country_bronce_medals(data_csv))))  
            elif opciones_internas == 5:
                print_nice(s_op.exclusion_ex_2_between_3_sets(set(s_comp.country_bronce_medals(data_csv)),set(s_comp.country_silver_medals(data_csv))))
